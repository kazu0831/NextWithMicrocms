...
const Category = async ({ params }: { params: { id: string } }) => {
    ...
    const { contents } = await client.get({ endpoint: 'blogs', queries: { filters: `category[equals]${params.id}` } })

    if (!contents || contents.length === 0) {
        return (
            <div className="container min-h-screen flex items-center justify-center mx-auto p-8">
